...
class CompanyBillInfoController extends Controller
{
    ...
    public function getinfo($id){
        ...
        $total=companyBillInfo::select(DB::raw('Sum(paid_amount)as paid'))
        ->where('company_bill_infos.company_bill_id1',$bill[0]->company_bill_id1)
        ->get();
      ...
      $totalss=$total1[0]->total-$total[0]->paid;
        ...
        <div style="width: 50%;text-align:right">
            <div class="form-group float-right">
                <label>Date: <strong id="bill_date1">'.date('Y-m-d h:i:s a', strtotime($bill[0]->created_at)).'</strong></label>
            </div>
        </div>
    </div>
    <div class="row p-4 table-sm table " style="margin-top: 20px">
        <span>Receiver Name :</span>
        <span><strong>'.$bill[0]->receiver_name.'</span></h5>
    ...
    <div class="row " style="margin-top:20px;width:100%">
        ...
        <table class="printablea4 table" id="testreport" style="width:100%">
            ...
            <tr>
                ...
                <td ><small >'.$bill[0]->paid_amount.'</small></td>
            </tr>
            <tr>
    ...
    <div class="row p-4" style="margin-top:20px;width:100%">
        <span>Issue By :</span>
        <span>'.$bill[0]->email.'</span>
    </div>    
</div>
<div style="margin-top:12px">
<img style="margin-left:45%" src="data:image/png;base64,'.DNS1D::getBarcodePNG(strval($bill[0]->bill_number), "C39",1,55,array(0,0,0), true).'"/>
<p style="text-align:center">Powerd By: PMS Medical Complex</p>
</div> ';
