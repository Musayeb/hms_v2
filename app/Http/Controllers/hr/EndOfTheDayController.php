...
class EndOfTheDayController extends Controller
{
    ...
    {
        $eod=EndOfTheDay::select('users.name as user_name','end_of_the_days.id','end_of_the_days.user_id','end_of_the_days.created_at','end_of_the_days.updated_at')
        ->join('users','users.id','end_of_the_days.user_id')->where('end_of_the_days.user_id',Auth::id())->orderBy('created_at','DESC')->paginate(50);
